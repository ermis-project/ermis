...
int64 nChainStartTime = 1416261027;
...
const string strMessageMagic = "Ermiscoin Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, CBlockIndex* pindex)
{
    int64_t nSubsidy;
    double nNetWeight = GetPoSKernelPS(pindex);
    if(nNetWeight < 21)
    {
	double rRate = 2.;		
	nSubsidy = (int64)((nCoinAge * rRate * CENT) * 33 / (365 * 33 + 8));
    }
    else
    {
	double rRate = 0.75*(log(nNetWeight/20));
	if (rRate > 10.) { rRate = 10.; }
	nSubsidy = (int64)((nCoinAge * rRate * CENT) * 33 / (365 * 33 + 8));
    }
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal ermiscoin miner
...
//            if (nStakeReward > GetProofOfStakeReward(nCoinAge, pindex->pprev)
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Ermiscoin
    bool fStrictPayToScriptHash = true; // Always active in Ermiscoin
    ...
    if (IsProofOfWork())
    {
        int64 nPoWReward = GetProofOfWorkReward(pindex->pprev->nHeight, nFees);
    ...
    if (IsProofOfStake())
    {
        ...
        int64 nPoSReward = GetProofOfStakeReward(nCoinAge, pindex->pprev);
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Ermiscoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "ERMIS forever initiated on 11/24/2014";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1416326400;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2914887;
	
        if (fTestNet)
        {
            block.nTime    = 1416261027;
            block.nNonce   = 24749056;
        }
        ...
        assert(block.hashMerkleRoot == uint256("708d572c966d9835f840ba217e0de0d6ced02dc80dfb646ffbf67e9ed0ff3fc4"));
...
unsigned char pchMessageStart[4] = { 0xf1, 0xb1, 0xb1, 0xd1 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("ErmiscoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("ErmiscoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("ErmiscoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("ErmiscoinMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        printf("Running ErmiscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
